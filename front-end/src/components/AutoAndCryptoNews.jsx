  ...
  return (
    <div className="bg-black text-white py-6 px-4 pb-10 relative">
      ...
      <div className="relative mt-4">
        ...
        <button
          onClick={scrollLeft}
          className="absolute -left-3 sm:left-2 top-1/2 transform -translate-y-1/2 bg-gray-800 p-1.5 sm:p-3 rounded-full z-10 text-white"
        >
          ◀
        </button>
<<<<<<< HEAD
...
>>>>>>> b57e4d16cdf555911647a66de29dc388982f38a1
        ...
        >
          {newsData.slice(0, 6).map((news) => (
            <div
              key={news._id}
              onClick={() =>
                navigate(`/news/${language}/${news.category}/${news.slug || news._id}`)
              }
        ...
        <button
          onClick={scrollRight}
          className="absolute -right-3 sm:right-2 top-1/2 transform -translate-y-1/2 bg-gray-800 p-1.5 sm:p-3 rounded-full z-10 text-white"
        >
          ▶
        </button>
      </div>
